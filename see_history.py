...
def main():
    ...
    p.add_argument("--history", required=True, help="path al file con la history (es. chess_elo_model (_VX_history.npy))")
    p.add_argument("--out", help="path output .txt (default sameprefix_history.txt)")
    p.add_argument("--start", type=int, default=1, help="starting version")
    p.add_argument("--end", type=int, default=-1, help="last version")
    args = p.parse_args()
    ...
    for idx in range(args.start, args.end + 1):    
        ...
        hist_path = f"{args.history}_V{idx}_history.npy"
